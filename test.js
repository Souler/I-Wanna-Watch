...
init(function() {
    var input = fs.readFileSync('out.json');
    input = JSON.parse(input);
    var links = input['streamin.to'];
    var wError = 0;
    async.eachSeries(links, function(link, cb) {
        console.log("Processing %s", link.href);
        videoSites.handle(link.href, function(err, videoUri) {
            if (err) {
                console.error("Error while getting: %s %s", link.href, videoUri);
                console.error(err);
                wError = wError + 1;
                return cb(null);
            }

            console.log("Got %s", videoUri);
            cb();
        })
    }, function(err) {
        console.log("Links with error: %d/%d", wError, links.length);
    })
})
return;
...
var pordede = require('./handlers/sites/seriesflv');
